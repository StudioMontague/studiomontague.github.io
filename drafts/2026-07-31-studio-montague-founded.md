# Studio Montague Begins
**Date:** 31 July 2026
**Status:** Draft — ready for review
**Main image:** `assets/coin-alpha-social.jpg` (upload/attach on Discord, X, IndieDB)
**Website inline image:** `assets/coin-alpha.png` (true transparency, sits directly on the dark page background — use this one if/when this post lives on studiomontague.dev)

---

## Website (studiomontague.dev)

### Studio Montague Begins

Today, Studio Montague officially begins.

I'm Graham Ainge — founder, and for now the only member, of a new independent game studio based in Brighton, UK. After a decade teaching science and a run touring as a keyboardist and vocalist in a band signed to Island Records, I'm starting again from the beginning: alone, at a desk, building a game.

That game is **Seed.Runner** (working title) — a 3D physics-based survival-adventure. Haul precious cargo across a barren world, where every measured mile leaves something behind. It's early. Right now it's pre-production: prototypes, systems, and a lot of paper. But it's real, and it's happening.

Studio Montague isn't a big studio, and it isn't trying to be one yet. It's one person, a home studio, and two whippets asleep in the corner — one of whom you'll recognise, cast in bronze, on the coin above. That coin is the first thing I've had made for the studio, and it felt right to mark day one with it.

Over the coming months I'll be sharing progress here, on Discord, and on IndieDB — prototypes, dead ends, decisions, and eventually a vertical slice. If you want to follow along from the start, this is the start.

More soon, including a proper look at Seed.Runner itself.

— Graham
Founder & Lead Developer, Studio Montague

---

## Discord

**Studio Montague begins today.**

Hey — I'm Graham, founder (and so far, only member) of Studio Montague, an independent studio based in Brighton, UK. This server is where I'll be sharing progress on the studio's first game as it happens: prototypes, decisions, dead ends, and eventually a vertical slice.

That game is **Seed.Runner** (working title) — a 3D physics-based survival-adventure. Haul precious cargo across a barren world, where every measured mile leaves something behind. It's early days — we're in pre-production — but it's real, and it's begun.

A bit about me: before this I spent over a decade teaching science, and before that I toured as a keyboardist and vocalist in a band signed to Island Records. Now it's just me, a home studio, and two whippets — one of whom is now immortalised on the studio's first coin, above.

Thanks for being here for day one. More soon.

— Graham

---

## X / Twitter

Today, Studio Montague begins.

I'm Graham — solo founder, Brighton UK. Building Seed.Runner: a 3D physics-based survival-adventure about hauling precious cargo across a barren world.

One person. One home studio. Two whippets. Coin minted for day one.

More soon.

---

## IndieDB

**Title:** Studio Montague Founded — First Look Coming Soon

Today marks the official founding of Studio Montague, an independent, solo game development studio based in Brighton, UK.

The studio's first title, Seed.Runner (working title), is a 3D physics-based survival-adventure: haul precious cargo across a barren world, where every measured mile leaves something behind. The project is currently in pre-production.

Studio Montague is a one-person operation, founded and led by Graham Ainge, who previously spent over a decade teaching science and before that toured as a keyboardist and vocalist in a band signed to Island Records. He works from a home studio in Brighton with two whippets — the inspiration behind the studio's mascot, cast here on Studio Montague's first commemorative coin.

Studio: Studio Montague
Founder: Graham Ainge
Location: Brighton, UK
First title: Seed.Runner (working title)
Genre: 3D physics-based survival-adventure
Status: Pre-production

Follow development on Discord and here on IndieDB as Seed.Runner moves toward a vertical slice.
